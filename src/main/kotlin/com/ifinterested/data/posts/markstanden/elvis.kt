...
val elvis =
    BlogPost.create(title = "Kotlin's Elvis Operator ?:",
                    body = listOf(PostElement(ValidPostElement.SUMMARY,
                                              "Kotlin's Elvis operator gets a fair bit of press and searching - but what is it, and why is it useful?"),
                                  PostElement(ValidPostElement.SUBTITLE, "TLDR"),
                                  ...
                                  PostElement(ValidPostElement.SUBTITLE, "Null-coalescing operator"),
                                  ...
                                  PostElement(ValidPostElement.PARAGRAPH,
                                              "In the code below, we check for "),
                                  PostElement(ValidPostElement.CODE, "val foundProduct = getProduct(searchTerm)\nval assetID: Int = if (foundProduct.id != null) foundProduct.id else 0"),
                                  PostElement(ValidPostElement.SUBTITLE, "Why Elvis?"),
                                  PostElement(ValidPostElement.PARAGRAPH,
